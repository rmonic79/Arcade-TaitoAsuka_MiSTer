...
## A note on analog geometry and the pause overlay

The CRT geometry controls (H-Size, V-Size with its PVM and Cabinet modes) and
the pause overlay are not part of this core. Doing them properly means editing
the framework in `sys/`, and the MiSTer-devel guidelines require that the
framework is left alone — while doing the geometry core-side would drag HDMI
along with the analog stretch, which is exactly what should not happen. They
live in a separate line of builds instead; see the end of this file.

What this core does provide, because it needs nothing from the framework, is
**Analog VGA H-Shift** (-32..+31 pixels) and **Analog VGA V-Shift** (0..7
lines): they delay the `VGA_HS` / `VGA_VS` pins only, so the sync the game sees
is untouched and HDMI — which goes through the scaler and regenerates its own
timing — does not change by a single pixel. Both default to 0. Pausing dims the
picture, and **Clean Pause** turns the dimming off.
...
| ![Eto Monogatari](docs/Ko_Map.png) | |
| Eto Monogatari — the board | |
...
```
Open TaitoAsuka.qpf in Quartus → Processing → Start Compilation
```

Output bitstream is generated in `output_files/TaitoAsuka.rbf`.
...
1. Copy the dated `.rbf` to `_Arcade/cores/` on the MiSTer SD card, named
   `TaitoAsuka.rbf` — that is the name the MRAs look for.
...
## CRT geometry and pause overlay: the `rm` line

Two features are deliberately **not** in this core: the CRT geometry controls
(H-Size, H-Position, V-Shift and V-Size, the last one with PVM and Cabinet
modes) and the pause overlay. Both require editing the framework in `sys/`,
which the MiSTer-devel guidelines do not allow.

They live in a parallel line of builds called **`rm`**, published separately:

- [rm-TaitoAsuka_MiSTer](https://github.com/rmonic79/rm-TaitoAsuka_MiSTer)

The `rm` build is this same core with those two additions, with the geometry
placed sys-side so that the scaler taps the video **before** it: the analog
output can be aligned to a tube while **HDMI stays bit-identical**. Its RBF is
named `rmTaitoAsuka`, different from the one here, so both can sit on the same
SD card without replacing each other.

Everything else — the emulation itself — is the same in both.
